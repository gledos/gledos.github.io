...
[ptime](http://www.pc-tools.net/win32/ptime/) 是一款开箱即用的简单 Windows 软件，以启动项传入要测试的内容就能得到耗时数据。

下表是测试耗时的数据，如果没有特别备注，那么耗时都是下载并合成一条龙的时间。
下载视频均使用 9.38 MB, 104 秒 的 niconico 视频 sm22577176，耗时使用多次下载，最快的结果。

| 软件           | 耗时     | 备注                   |
| -------------- | -------- | ---------------------- |
| N_m3u8DL-CLI   | 4.394 s  | 此软件仅适用于 Windows |
| hlsdl          | 41.291 s |                        |
| streamlink     | 42.939 s |                        |
| ffmpeg         | 44.284 s |                        |
| streamlink     | 44.713 s |                        |
| youtube-dl/dlc | 48.635 s |                        |

<!-- 虽然不同工具得出的视频文件大小有差异，不过在 MediaInfo 里看不出实质区别。 -->
...
+ 通过录制快速获取带有弹幕效果的视频
  + (实际上将弹幕转为字幕文件，然后根据需求「烘焙」字幕或者外挂字幕，这样的效果是最好的)
...
### 使用 N_m3u8DL-CLI 进行下载

[nilaoda/N_m3u8DL-CLI](https://github.com/nilaoda/N_m3u8DL-CLI) 是一款仅支持 Windows 系统的 CLI 工具。

速度与方便程度应该是最好的，因为使用了多线程与自动重连。

使用方法有中文指南，所以这里就不介绍了。

搭配浏览器扩展 [stream-detector](https://github.com/rowrawer/stream-detector/) 使用效果最佳。
...
>
>最后作者在官网声明 SSTap 和 SocksCap64 于 2017年11月19日 停止开发及维护和下载，Taro 的 SSCap 也停止了维护。
>
...
1. 菜单中选择 `服务器(Server)`，如果你常用的代理软件已经给出了 Socks5 端口，
   就可以直接在 `添加 [Socks5] 服务器` 选项中填写，进行伪·双重代理
...
通常使用上述的 FFmpeg 教程就能直接合成，但如果你在学会使用之前，就下载了大量的 .ts 视频片段，这时就可以使用下面介绍的方法了。

ts 格式设计之初，就易于合并，将二进制视频数据首尾相连就能合并。
...
假设只有小于等于 9 个文件，比如 1.ts 2.ts ...... 9.ts ，只要使用星号就能按顺序合成，
但如果超过 9 个文件，和成时就会把 `10` 排在 `2` 的前面，导致顺序错误。
...
比如有 200 个 ts 文件，就需要对 99.ts 及其之前的文件进行补0，使文件列表变成下表所示。
